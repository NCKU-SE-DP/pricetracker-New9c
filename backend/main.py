...
def get_new(is_initial=False):
    ...
    for news in news_data:
        title = news["title"]
        tell_relevance= [ # tells the relevance 
            {
                "role": "system",
        ...
        if relevance == "high":
            ...
            paragraphs = [
                ...
                if paragraph.text.strip() != "" and "▪" not in p.text
            ]
            detailed_news =  {
                "url": news["titleLink"],
                "title": title,
...
def verify(inserted_pwd, hashed_pwd): #check if inserted password == hashed password
    return pwd_context.verify(inserted_pwd, hashed_pwd)


def check_user_password_is_correct(db, username, pwd):
    OuO = db.query(User).filter(User.username == username).first()
...
def get_article_upvote_details(article_id, user_id, db): #make it more understandable
...
def read_news(db=Depends(session_opener)):
    ...
    result = []
    for article in news: # Approve readability 
...
async def search_news(request: PromptRequest):
    ...
    news_list = []
    keyword_grabber= [ #prompt grabs keywords
        {
            "role": "system",
            "content": "你是一個關鍵字提取機器人，用戶將會輸入一段文字，表示其希望看見的新聞內容，請提取出用戶希望看見的關鍵字，請截取最重要的關鍵字即可，避免出現「新聞」、「資訊」等混淆搜尋引擎的字詞。(僅須回答關鍵字，若有多個關鍵字，請以空格分隔)",
        },
...
async def news_summary(
    ...
    response = {}
    summary_prompt = [ # says what it does
        {
            "role": "system",
            "content": "你是一個新聞摘要生成機器人，請統整新聞中提及的影響及主要原因 (影響、原因各50個字，請以json格式回答 {'影響': '...', '原因': '...'})",
        },
...
def news_exists(news_id, db: Session): # id2 does not say what it does
